<div class="max-w-7xl mx-auto space-y-8">
    ...
    <div class="max-w-5xl mx-auto mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h2 class="text-3xl font-bold tracking-tight text-slate-900 dark:text-white">{{ __('ui.create_tenant_account') }}</h2>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Setup a new workspace and configure subscription details.</p>
        </div>
    ...
    <!-- Main Form Card -->
    <div class="max-w-5xl mx-auto bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700/60 overflow-hidden relative">

        <!-- Loading Overlay -->
        <div wire:loading wire:target="createTenant" class="absolute inset-0 bg-white/50 dark:bg-slate-900/50 z-50 backdrop-blur-sm flex items-center justify-center">
        ...
        <form wire:submit.prevent="createTenant" class="p-6 md:p-8 space-y-10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div class="relative group">
                            ...
                            <input type="text"
                                   wire:model.live.debounce.500ms="tenantName"
                        ...
                        @error('tenantName') <span class="text-rose-500 text-xs font-medium flex items-center mt-1"><x-heroicon-s-exclamation-circle class="w-3 h-3 mr-1"/>{{ $message }}</span> @enderror
                    </div>

                    <!-- Domain Preview (Input Group) -->
                    <div class="space-y-1.5">
                        <label class="text-sm font-medium text-slate-700 dark:text-slate-300">System Domain</label>
                        <div class="flex rounded-xl shadow-sm ring-1 ring-inset ring-slate-200 dark:ring-slate-700 overflow-hidden bg-slate-50 dark:bg-slate-900/50">
                            <span class="flex select-none items-center pl-3 text-slate-400 sm:text-sm">https://</span>
                            <input type="text"
                                   wire:model="generatedDomain"
                                   readonly
                            ...
                            <span class="flex select-none items-center pr-3 text-slate-400 sm:text-sm border-l border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 px-3">.app.com</span>
                        </div>
                    ...
                    <!-- Admin Email -->
                    <div class="space-y-1.5">
                        <label class="text-sm font-medium text-slate-700 dark:text-slate-300">Administrator Email</label>
                        <div class="relative group">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-400 group-focus-within:text-indigo-500 transition-colors">
                        ...
                        @error('adminEmail') <span class="text-rose-500 text-xs font-medium flex items-center mt-1"><x-heroicon-s-exclamation-circle class="w-3 h-3 mr-1"/>{{ $message }}</span> @enderror
                    </div>
                </div>
            </section>
            ...
            <section>
                <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
                    <div>
                        ...
                        <p class="text-sm text-slate-500 dark:text-slate-400 mt-1 ml-10">Choose the best plan for the organization.</p>
                    </div>

                    <!-- Modern Toggle Switch -->
                    <div class="bg-slate-100 dark:bg-slate-900 p-1.5 rounded-xl flex items-center self-start sm:self-center ring-1 ring-inset ring-slate-200 dark:ring-slate-700">
                        <button type="button" wire:click="$set('billingCycle', 'monthly')"
                            ...
                            @if($billingCycle === 'monthly')
                                <div class="absolute inset-0 bg-white dark:bg-slate-700 rounded-lg shadow-sm -z-10" layoutId="highlight"></div>
                            @endif
                            Monthly
                        </button>
                        <button type="button" wire:click="$set('billingCycle', 'yearly')"
                            ...
                            @if($billingCycle === 'yearly')
                                <div class="absolute inset-0 bg-white dark:bg-slate-700 rounded-lg shadow-sm -z-10" layoutId="highlight"></div>
                            @endif
                            Yearly <span class="ml-1 text-[10px] text-emerald-600 font-bold bg-emerald-100 dark:bg-emerald-500/20 px-1.5 py-0.5 rounded-full">-20%</span>
                        </button>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                    @foreach($plans ?? [] as $plan)
                        ...
                        <div wire:click="$set('subscriptionTier', '{{ $plan['id'] }}')"
                            ...
                            @if($isSelected)
                                <div class="absolute -top-3 left-1/2 transform -translate-x-1/2 bg-indigo-600 text-white text-[10px] font-bold px-3 py-1 rounded-full shadow-sm uppercase tracking-wider">
                                    Selected
                                </div>
                            @endif

                            <div class="p-5 flex-1">
                                <h4 class="font-bold text-base {{ $isSelected ? 'text-indigo-900 dark:text-indigo-100' : 'text-slate-900 dark:text-white' }}">
                                    {{ $plan['name'] }}
                                </h4>
                                ...
                                <div class="mt-3 mb-6">
                                    ...
                                    <span class="text-xs font-medium text-slate-500 dark:text-slate-400">FCFA / {{ $billingCycle == 'monthly' ? 'mo' : 'yr' }}</span>
                                </div>

                                <div class="space-y-3">
                                    <!-- Feature: Users -->
                                    <div class="flex items-start text-xs text-slate-600 dark:text-slate-300">
                                        <div class="mt-0.5 mr-2 p-1 rounded-full {{ $isSelected ? 'bg-indigo-200 dark:bg-indigo-500/30 text-indigo-700 dark:text-indigo-300' : 'bg-slate-100 dark:bg-slate-700 text-slate-500' }}">
                                            <x-heroicon-s-users class="w-3 h-3" />
                                        </div>
                                        <span class="leading-5">
                                            <strong class="font-semibold text-slate-900 dark:text-white">{{ $plan['features']['max_users'] == -1 ? 'Unlimited' : $plan['features']['max_users'] }}</strong> Users
                                        </span>
                                    </div>
                                    ...
                                    <div class="flex items-start text-xs text-slate-600 dark:text-slate-300">
                                        ...
                                        <span class="leading-5">
                                            <strong class="font-semibold text-slate-900 dark:text-white">{{ $plan['features']['max_storage'] == -1 ? 'Unlimited' : ($plan['features']['max_storage'] / 1024) . ' GB' }}</strong> Storage
                                        </span>
                                    </div>

                                    <!-- Feature: API -->
                                    <div class="flex items-start text-xs text-slate-600 dark:text-slate-300">
                                        <div class="mt-0.5 mr-2 p-1 rounded-full {{ $plan['features']['api_access'] ? ($isSelected ? 'bg-indigo-200 dark:bg-indigo-500/30 text-indigo-700 dark:text-indigo-300' : 'bg-slate-100 dark:bg-slate-700 text-slate-500') : 'bg-slate-50 dark:bg-slate-800 text-slate-300' }}">
                                        ...
                                        <span class="leading-5 {{ !$plan['features']['api_access'] ? 'text-slate-400 dark:text-slate-600 line-through' : '' }}">
                                            API Access
                                        </span>
                                    </div>
                                </div>
                            ...
                            <!-- Radio Indicator -->
                            <div class="px-5 py-3 border-t {{ $isSelected ? 'border-indigo-200 dark:border-indigo-500/30 bg-indigo-50/50 dark:bg-indigo-600/5' : 'border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50' }}">
                                <div class="flex items-center justify-center">
                                    <div class="h-4 w-4 rounded-full border {{ $isSelected ? 'border-indigo-600 bg-indigo-600' : 'border-slate-300' }} flex items-center justify-center">
                                        @if($isSelected) <div class="h-1.5 w-1.5 rounded-full bg-white"></div> @endif
                                    </div>
                                    <span class="ml-2 text-xs font-semibold {{ $isSelected ? 'text-indigo-700 dark:text-indigo-300' : 'text-slate-500' }}">
            ...
            <div class="pt-6 flex items-center justify-end gap-3 border-t border-slate-100 dark:border-slate-700">
                <button type="button" class="px-5 py-2.5 text-sm font-medium text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-xl shadow-sm hover:bg-slate-50 dark:hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all">
                    Cancel
                </button>
                ...
                <button type="submit"
                    ...
                    <x-heroicon-s-arrow-right class="w-4 h-4 ml-2 group-hover:translate-x-1 transition-transform" wire:loading.remove />
                </button>
            </div>
        </form>
    </div>
