<div class="flex-1 h-full relative"
    ...
    x-data="{
        ...
        init() {
            ...
            window.addEventListener('online', () => {
                ...
                // Hide the 'Back Online' toast after 4 seconds
    ...
    <!-- 1. Offline Alert (Persistent while offline) -->
    <div x-show="isOffline"
         x-transition.opacity
         class="fixed top-0 inset-x-0 z-[60] bg-rose-500 text-white px-4 py-3 shadow-md flex items-center justify-center gap-2 transform transition-transform duration-300">
        ...
        <span class="text-sm font-medium">You are currently offline. Changes will not be saved.</span>
    </div>

    <!-- 2. Online Toast (Temporary popup when connection returns) -->
    <div x-show="showOnlineToast"
         ...
         class="fixed top-4 left-1/2 transform -translate-x-1/2 z-[60] bg-emerald-600 text-white px-6 py-3 rounded-full shadow-lg flex items-center gap-2">
        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.111 16.404a5.5 5.5 0 017.778 0M12 20h.01m-7.08-7.071c3.904-3.905 10.236-3.905 14.141 0M1.394 9.393c5.857-5.857 15.355-5.857 21.213 0" />
        </svg>
        <span class="text-sm font-bold">Connection restored</span>
    </div>

    <!--
      CONTENT WRAPPER
    -->
    <div class="max-w-full mx-auto space-y-4 p-2 md:p-4 transition-all duration-300"
         :class="{ 'pt-14': isOffline }"> <!-- Push content down when offline banner is showing -->
        ...
        <header class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
            ...
            <div class="flex items-center gap-2 w-full md:w-auto">
                <div class="relative group w-full md:w-40">
                    <select
                        class="w-full appearance-none bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 pl-3 pr-8 py-2 text-xs font-medium text-slate-700 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all cursor-pointer">
                    ...
                    <div
                        class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-slate-400 group-hover:text-indigo-500 transition-colors">
        ...
        <section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            @foreach ($stats as $stat)
                @php
                    ...
                    $trendIcon =
                        $stat['trend'] === 'up' ? 'M13 7h8m0 0v8m0-8l-8 8-4-4-6 6' : 'M13 17h8m0 0V9m0 8l-8-8-4 4-6-6';
                @endphp

                <div
                    class="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md transition-shadow duration-300 group">
                    <div class="flex justify-between items-start">
                        <div>
                            <p
                                class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                                {{ __($stat['title']) }}</p>
                            <h3 class="text-2xl font-bold text-slate-900 dark:text-white mt-1 tracking-tight">
                                {{ $stat['value'] }}</h3>
                        </div>
                        <div
                            class="p-2 rounded-lg {{ $colors['bg'] }} {{ $colors['text'] }} transition-transform group-hover:scale-110 duration-300">
                            <x-dynamic-component :component="'heroicon-o-' . $stat['icon']" class="h-5 w-5" />
                        </div>
                    </div>
                    <div class="flex items-center mt-3">
                        <span
                            class="flex items-center text-[10px] font-bold {{ $trendColor }} bg-slate-50 dark:bg-gray-700/50 px-1.5 py-0.5 rounded">
        ...
        <section
            ...
            <div class="px-4 py-3 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
                ...
                <a href="#"
                    class="text-xs font-semibold text-indigo-600 hover:text-indigo-500 dark:text-indigo-400 flex items-center transition">
                    {{ __('ui.view_all') }}
                    ...
                    <thead class="bg-gray-50 dark:bg-gray-900/50">
                        <tr>
                            @foreach (['tenant_name', 'hospital_name', 'domain', 'subscription', 'joined_on'] as $head)
                                <th scope="col"
                                    class="px-4 py-2 text-left text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                                    {{ __('ui.' . $head) }}</th>
                            @endforeach
                            <th scope="col" class="relative px-4 py-2"><span
                                    class="sr-only">{{ __('ui.actions') }}</span></th>
                        </tr>
                    </thead>
                            ...
                            <tr class="group hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                                <td class="px-4 py-2.5 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div
                                            class="h-8 w-8 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold text-xs shadow-sm ring-2 ring-white dark:ring-gray-800">
                                            {{ substr($tenant['name'], 0, 1) }}
                                        </div>
                                        <div class="ml-3">
                                            <div class="text-xs font-semibold text-slate-900 dark:text-white">
                                                {{ $tenant['name'] }}</div>
                                            <div class="text-[10px] text-slate-500 dark:text-slate-400">ID:
                                                #{{ $tenant['id'] ?? rand(100, 999) }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-2.5 whitespace-nowrap text-xs text-slate-600 dark:text-slate-300">
                                    {{ $tenant['hospital_name'] }}</td>
                                <td
                                    class="px-4 py-2.5 whitespace-nowrap text-xs text-indigo-500 hover:underline cursor-pointer">
                                    {{ $tenant['domain'] }}</td>
                                <td class="px-4 py-2.5 whitespace-nowrap">
                                    @php
                                    ...
                                    @endphp
                                    <span
                                        class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium border {{ $badge }}">
                                        <span class="w-1 h-1 rounded-full mr-1.5 {{ $dot }}"></span>
                                        {{ $sub }}
                                    </span>
                                </td>
                                <td class="px-4 py-2.5 whitespace-nowrap text-xs text-slate-500">
                                    {{ $tenant['created_at'] }}</td>
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('dashboardChart', (config) => ({
            ...
            initChart() {
                ...
                200); // Shorter gradient for shorter chart
                ...
                this.chart = new Chart(ctx, {
                    ...
                    data: {
                        ...
                        datasets: [{
                            ...
                            backgroundColor: config.type === 'bar' ? config.color :
                                gradient,
                            borderWidth: 2,
                            borderRadius: 3,
