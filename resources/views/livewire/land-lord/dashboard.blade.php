<div class="flex-1 bg-gray-50 dark:bg-gray-900 font-sans" x-data="{
    ...
    <header
        class="sticky top-0 lg:top-0 z-20 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md border-b border-gray-200 dark:border-gray-800 px-4 sm:px-6 py-4">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            {{-- Right: Filters & Dark Mode Toggle --}}
    ...
    <div class="p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        <section class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            ...
            @foreach ($stats as $stat)
                <div
                    class="bg-white dark:bg-gray-800 p-5 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm transition hover:shadow-md">
                    <div class="flex justify-between">
                        ...
                        <div
                            class="p-2 bg-{{ $stat['color'] }}-50 dark:bg-{{ $stat['color'] }}-900/20 text-{{ $stat['color'] }}-600 dark:text-{{ $stat['color'] }}-400 rounded-lg">
        ...
        <section class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div wire:ignore
                class="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm h-80 flex flex-col"
                x-data="adminChart({
                    type: 'line',
                    color: '#6366f1',
                    label: 'Revenue',
                        ...
                        @foreach (range(1, 3) as $i)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-8 h-8 rounded bg-indigo-100 dark:bg-indigo-900 flex items-center justify-center text-xs font-bold text-indigo-700 dark:text-indigo-300">
                                            H</div>
                                        <span class="text-sm font-medium dark:text-white">Hospital Center
                                            {{ $i }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-sm">
                                    <span
                                        class="px-2 py-1 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 text-[10px] font-bold rounded-full uppercase">Active</span>
                                </td>
    ...
    {{-- Online Toast --}}
    <div x-show="showOnlineToast" x-transition
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('adminChart', (config) => ({
            ...
            init() {
                this.render();
                // Watch for dark mode changes to redraw chart with new colors
                this.$watch('darkMode', () => this.render());
            },
            render() {
