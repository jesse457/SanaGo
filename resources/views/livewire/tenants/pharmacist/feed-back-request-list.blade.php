             ...
             <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                 ...
                 <li>
                     <div class="flex items-center">
                         ...
                         <span class="ms-1 text-sm font-medium text-gray-400 md:ms-2 dark:text-gray-300">{{ __('feedback.breadcrumb_feedback_history') }}</span>
                     </div>
                 </li>
             </ol>
         </nav>
     ...
     <header class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
         <div>
             <h1 class="text-3xl font-bold text-gray-800 dark:text-white mb-1 flex items-center gap-3">
                 ...
                 {{ __('feedback.feedback_history_title') }}
             </h1>
             <p class="text-gray-600 dark:text-gray-400">{{ __('feedback.feedback_history_description') }}</p>
         </div>
         ...
         <div class="flex items-center gap-3">
             <a href="{{ route('pharmacist.submit-feedback') }}" wire:navigate
                 ...
                 <x-heroicon-o-plus class="w-5 h-5" /> {{ __('feedback.submit_feedback_button') }}
             </a>
         </div>
     </header>
     ...
     <div class="bg-white rounded-xl shadow-lg border border-gray-100 p-6 mb-10 dark:bg-gray-800 dark:border-gray-700">
         ...
         @if (!isset($feedbacks) || $feedbacks->total() === 0)
             <div class="text-center py-12 text-gray-500 dark:text-gray-300">
                 ...
                 <p class="text-lg font-medium">{{ __('feedback.no_feedback_found') }}</p>
                 <p class="mt-2 text-sm">{{ __('feedback.submit_first_feedback') }}</p>
                 <div class="mt-4">
                     <a href="{{ route('pharmacist.submit-feedback') }}" wire:navigate
                         class="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white font-medium rounded-md hover:bg-indigo-700 transition duration-150 ease-in-out shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                         <x-heroicon-o-plus class="w-5 h-5" /> {{ __('feedback.submit_feedback_button') }}
         ...
         @else
                             ...
                             <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                 <td class="px-4 py-4 whitespace-nowrap">
                                     <div class="text-sm font-medium text-gray-900 dark:text-white">
                                         {{ $feedback->subject ?? __('feedback.no_subject') }}</div>
                                 ...
                                 <td class="px-4 py-4 whitespace-nowrap text-right text-sm font-medium">
                                     <div class="flex items-center justify-end gap-2">
                                         <button wire:click.prevent="showFeedback({{ $feedback->id }})"
                                             ...
                                             <x-heroicon-s-eye class="w-4 h-4 text-gray-600 dark:text-gray-200" /> {{ __('feedback.view_button') }}
                                         </button>
             ...
             <div class="mt-4 flex items-center justify-between gap-4">
                 <div class="text-sm text-gray-600 dark:text-gray-300">
                     {{ __('feedback.showing_to_of', ['first' => $feedbacks->firstItem() ?? 0, 'last' => $feedbacks->lastItem() ?? 0, 'total' => $feedbacks->total() ?? 0]) }}
                 </div>
                ...
                <div role="dialog" aria-modal="true" aria-labelledby="modal-title"
                    ...
                    <div class="px-6 py-4 border-b dark:border-gray-700 flex justify-between items-center">
                        <h3 id="modal-title" class="text-lg font-semibold text-gray-900 dark:text-white">
                            {{ $modalTitle ?? __('feedback.feedback_details') }}</h3>
         ...
         <div class="space-y-6">
             ...
             <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                 <div class="min-w-0">
                     ...
                     <div class="mt-2 text-sm text-gray-500 dark:text-gray-400 flex flex-wrap items-center gap-2">
                         ...
                         <span class="text-xs text-gray-400">
                             {{ __('feedback.submitted_ago', ['time' => optional($modalFeedback->created_at)->diffForHumans()]) }}
                         </span>
                 ...
                 <div class="text-right hidden sm:block">
                     <div class="text-xs text-gray-500 dark:text-gray-300">{{ __('feedback.status') }}</div>
                     <div class="mt-1">
                         <span
             ...
             {{-- Published response --}}
             <div>
                 <h5 class="text-sm font-medium text-gray-800 dark:text-gray-100">{{ __('feedback.response') }}</h5>
                         ...
                         <div class="flex items-start gap-3">
                             ...
                             <div class="min-w-0">
                                 ...
                                 <div class="text-xs text-gray-400 mt-3">
                                     {{ __('feedback.responded_by', ['name' => optional($modalFeedback->responder)->name ?? 'Staff']) }} •
                 ...
                 @else
                     <div
                         ...
                         <div class="font-medium">{{ __('feedback.no_response_yet') }}</div>
                         <div class="text-xs mt-1">{{ __('feedback.use_reply_area') }}</div>
                     </div>
                 @endif
             </div>
             ...
             <div class="flex gap-2 mt-4 justify-end">
                 <button @click="open = false; $wire.closeModal();"
                     ...
                     {{ __('feedback.close_button') }}
                 </button>
             </div>
         </div>
     @else
         <div class="text-center py-6 text-gray-500 dark:text-gray-300">{{ __('feedback.loading') }}</div>
         @endif
     </div>
     </div>
     </div>
