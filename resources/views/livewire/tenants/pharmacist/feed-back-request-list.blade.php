                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                            {{ __('View and track the status of your submitted feedback.') }}
                        </p>
                    </div>
                </div>
                ...
                @if (!isset($feedbacks) || $feedbacks->total() === 0)
                    <div class="text-center py-16 px-6">
                        ...
                        <p class="text-slate-500 dark:text-slate-400 max-w-sm mx-auto mb-8">
                            {{ __('Submit your first feedback to let us know how we can improve. Your contribution helps us provide a better experience for everyone.') }}
                        </p>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-slate-100 dark:divide-gray-800">
                            <thead class="bg-slate-50 dark:bg-gray-950">
                                <tr>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">{{ __('Subject') }}</th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider hidden sm:table-cell">{{ __('Category') }}</th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">{{ __('Status') }}</th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider hidden md:table-cell">{{ __('Submitted') }}</th>
                                    ...
                                    <div class="space-y-6">
                                        ...
                                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-50 dark:bg-gray-800/50 p-4 rounded-xl border border-slate-100 dark:border-gray-800">
                                            <div class="flex flex-wrap items-center gap-3">
                                                ...
                                                <span class="text-xs font-medium text-slate-500 dark:text-slate-400 flex items-center gap-1">
                                                    ...
                                                    {{ __('Submitted') }} {{ optional($modalFeedback->created_at)->diffForHumans() }}
                                                </span>
                                            </div>
                                                ...
                                                <div class="flex-grow">
                                                    <div class="bg-blue-50/50 dark:bg-blue-900/10 rounded-2xl rounded-tl-none p-5 border border-blue-100 dark:border-blue-900/30">
                                                        ...
                                                        <div class="flex items-center gap-2 text-xs text-slate-400 font-medium">
                                                            <span>{{ __('Responded by') }} <span class="text-slate-600 dark:text-slate-300 font-bold">{{ optional($modalFeedback->responder)->name ?? __('Staff') }}</span></span>
                                                            <span>•</span>
                                        ...
                                        @else
                                            <div class="text-center py-8 bg-slate-50 dark:bg-gray-800/50 rounded-2xl border border-dashed border-slate-300 dark:border-gray-700">
                                                ...
                                                <p class="text-sm font-medium text-slate-500 dark:text-slate-400">{{ __('No response yet. Staff will review your feedback soon.') }}</p>
                                            </div>
                                        @endif
                                    </div>
                                @else
