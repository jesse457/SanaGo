                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <span class="ms-1 text-sm text-gray-400 md:ms-2 dark:text-gray-200">{{ __('Submit Feedback') }}</span>
                        </div>
                    </li>
                </ol>
            </nav>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div
            ...
            <p class="text-md text-blue-600 dark:text-blue-400 font-medium mb-3">
                {{ ucfirst(__(Auth::user()->role ?? 'User')) }}</p>
            ...
            <div class="mt-6 w-full text-left border-t border-gray-200 dark:border-gray-700 pt-6">
                <h3 class="text-lg font-semibold text-gray-800 dark:text-white mb-3">{{ __('Quick Info') }}</h3>
                <div class="space-y-2 text-sm text-gray-700 dark:text-gray-300">
                    <p class="flex items-center"><x-heroicon-o-phone class="w-5 h-5 mr-2 text-gray-500" /> {{ __('Phone:') }}
                        <span class="ml-auto font-medium">{{ $phone_number ?? __('N/A') }}</span>
                    </p>
                    <p class="flex items-center"><x-heroicon-o-map-pin class="w-5 h-5 mr-2 text-gray-500" /> {{ __('Address:') }}
                        <span class="ml-auto font-medium">{{ $address ?? __('N/A') }}</span>
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-xl rounded-xl p-6 border border-gray-100 dark:border-gray-700">
                <h3 class="text-xl font-semibold text-gray-800 dark:text-white mb-6 border-b pb-3">{{ __('Personal Information') }}
                </h3>
                <form wire:submit.prevent="updateProfile" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="col-span-2 flex justify-end">
                            <button type="submit"
                                class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">{{ __('Update Profile') }}</button>
                        </div>
                </form>
            </div>
            ...
            {{-- Change Password --}}
            <div class="bg-white dark:bg-gray-800 shadow-xl rounded-xl p-6 border border-gray-100 dark:border-gray-700">
                <h3 class="text-xl font-semibold text-gray-800 dark:text-white mb-6  pb-3">{{ __('Change Password') }}</h3>
                <form wire:submit.prevent="updatePassword" class="space-y-4">
                    <div>
                        ...
                        <div class="flex justify-end">
                            <button type="submit"
                                class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">{{ __('Update Password') }}</button>
                        </div>
                </form>
            </div>
        </div>
