<div class="flex-1 p-4 md:p-6 lg:ml-64 bg-gray-50 dark:bg-gray-900 overflow-y-auto min-h-screen">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                <li class="inline-flex items-center">
                    <a href="{{ route('pharmacist.dashboard') }}" wire:navigate
                        ...
                        <x-heroicon-s-home class="w-4 h-4 me-2.5" />{{ __('Home') }}
                    </a>
                </li>
                <li>
                    <div class="flex items-center">
                        <x-heroicon-s-chevron-right class="rtl:rotate-180 w-3 h-3 text-gray-400 mx-1" />
                        <span class="ms-1 text-sm font-medium text-gray-700 md:ms-2 dark:text-gray-400">{{ __('Sales Report') }}</span>
                    </div>
                </li>
            </ol>
        </nav>
    ...
    <h2 class="text-3xl sm:text-4xl font-extrabold text-gray-800 dark:text-white mb-8">
        {{ __('Sales Report') }}
        <span class="block text-base font-normal text-gray-500 mt-1 dark:text-gray-400">{{ __('Generate reports of medications sold over time.') }}</span>
    </h2>

    <div class="bg-white rounded-2xl shadow p-6 mb-6 dark:bg-gray-800">
        <h3 class="text-2xl font-semibold text-gray-800 mb-4 dark:text-white">{{ __('Top Selling Medications') }}</h3>
        ...
        <div class="flex flex-col sm:flex-row justify-between mb-4 space-y-3 sm:space-y-0 sm:space-x-4">
            ...
            <input type="text" wire:model.live.debounce.400ms="search"
                placeholder="{{ __('Search by medication name...') }}"
            ...
            <div x-data="{}" class="flex space-x-2">
                <button @click="@this.call('downloadCsv')"
                    ...
                    <x-heroicon-s-arrow-down-tray class="w-4 h-4 mr-2" /> {{ __('Download CSV') }}
                </button>
                <button @click="@this.call('downloadExcel')"
                    ...
                    <x-heroicon-s-arrow-down-tray class="w-4 h-4 mr-2" /> {{ __('Download Excel') }}
                </button>
            </div>
        </div>
        ...
        <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-700">
                    <tr>
                        <th class="px-4 py-2 text-left text-sm font-medium text-gray-700 dark:text-gray-200">{{ __('Rank') }}</th>
                        <th class="px-4 py-2 text-left text-sm font-medium text-gray-700 dark:text-gray-200">{{ __('Medication Name') }}</th>
                        <th class="px-4 py-2 text-left text-sm font-medium text-gray-700 dark:text-gray-200">{{ __('Quantity Sold') }}</th>
                    </tr>
                </thead>
                <tbody id="topSellingMedsBody" class="bg-white divide-y divide-gray-100 dark:bg-gray-800 dark:divide-gray-700">
                    @forelse($topSellingMedications as $index => $med)
                        <tr>
                            ...
                            <td class="px-4 py-2 text-gray-800 dark:text-gray-200">{{ $med->total_quantity_sold }} {{ __('units') }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="3" class="px-4 py-2 text-center text-sm text-gray-500 dark:text-gray-400">{{ __('No top selling medications data available.') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
