                ...
                <div class="flex flex-col gap-2">
                    ...
                    <nav class="flex text-xs font-medium text-gray-500 dark:text-gray-400 mb-1 mt-2"
                        ...
                        <ol class="inline-flex items-center space-x-1 md:space-x-2">
                            ...
                            <li>
                                <div class="flex items-center">
                                    <x-heroicon-s-chevron-right class="w-3 h-3 text-gray-300 mx-1" />
                                    <span class="text-gray-900 dark:text-white">{{ __('Submit Feedback') }}</span>
                                </div>
                            </li>
                        </ol>
                    </nav>
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400 max-w-3xl">
                            {{ __('Your thoughts are important! Help us make it better.') }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class=" mx-auto">
                <div
                    ...
                    <div
                        ...
                        <div>
                            <h3 class="text-base font-bold text-slate-900 dark:text-white">{{ __('Help Us Make It Better') }}</h3>
                            <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">{{ __('Your thoughts are important! Tell us what you think to help us improve.') }}</p>
                        </div>
                        <span
                            class="hidden sm:inline-flex items-center rounded-full bg-pink-50 dark:bg-pink-900/30 px-3 py-1 text-xs font-bold text-pink-700 dark:text-pink-300 border border-pink-100 dark:border-blue-800">SanaGo</span>
                    </div>
                    ...
                    <form wire:submit="submit" class="p-6 sm:p-8 space-y-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-1.5">
                                <label for="subject"
                                    class="block text-sm font-bold text-slate-700 dark:text-slate-300">{{ __('What is this about?') }} <span class="text-red-500">*</span></label>
                                <input id="subject" type="text" wire:model.defer="subject"
                                    placeholder="{{ __('e.g., I have a problem with appointments') }}"
                            ...
                            <div class="space-y-1.5">
                                <label for="category"
                                    class="block text-sm font-bold text-slate-700 dark:text-slate-300">{{ __('Which part of the system?') }} <span class="text-red-500">*</span></label>
                                <select id="category" wire:model.defer="category"
                                    ...
                                    <option value="appointments">{{ __('Appointments') }}</option>
                                    <option value="book-appointment">{{ __('Book Appointments') }}</option>
                                    <option value="patients">{{ __('Patients') }}</option>
                                    <option value="register-patient">{{ __('Register Patients') }}</option>
                                    <option value="patient-admission">{{ __('Patient Admissions') }}</option>
                                    <option value="admit-patient">{{ __('Admit Patient') }}</option>
                                    <option value="other">{{ __('Other / General Inquiry') }}</option>
                                </select>
                        ...
                        <div class="space-y-1.5">
                            <label for="message"
                                class="block text-sm font-bold text-slate-700 dark:text-slate-300">{{ __('Tell us what happened') }}
                                <span class="text-red-500">*</span></label>
                            <textarea id="message" rows="6" wire:model.defer="message"
                                placeholder="{{ __('Please describe what you experienced in your own words. The more details you give us, the better we can help.') }}"
                            ...
                            <div class="flex justify-between items-start mt-1">
                                ...
                                <p class="text-xs text-slate-500 dark:text-slate-400 text-right">
                                    <b>{{ __('Tip:') }}</b> {{ __('Tell us what you were trying to do, what happened, and what you expected to happen instead.') }}
                                </p>
                            </div>
                        </div>
                        ...
                        <div
                            ...
                            <div class="text-xs text-slate-500 dark:text-slate-400">
                                {{ __('By sending this, you agree to our') }} <a href="#"
                                    class="text-blue-600 hover:underline font-medium">{{ __('rules') }}</a> {{ __('and') }} <a href="#"
                                    class="text-blue-600 hover:underline font-medium">{{ __('privacy policy') }}</a>.
                            </div>
                            <div class="flex flex-col-reverse sm:flex-row gap-3 w-full sm:w-auto">
                                ...
                                <button type="submit"
                                    class="w-full sm:w-auto inline-flex items-center justify-center px-6 py-2.5 rounded-xl text-white bg-gradient-to-tr from-pink-500 to-rose-500 hover:bg-blue-700 shadow-md hover:shadow-lg text-sm font-bold transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
