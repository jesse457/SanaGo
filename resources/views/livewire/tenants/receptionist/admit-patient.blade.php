                ...
                <div class="flex flex-col gap-2">
                    ...
                    <nav class="flex text-xs font-medium text-gray-500 dark:text-gray-400 mb-1" aria-label="Breadcrumb">
                        <ol class="inline-flex items-center space-x-1 md:space-x-2">
                            ...
                            <li>
                                <div class="flex items-center">
                                    <x-heroicon-s-chevron-right class="w-3 h-3 text-gray-300 mx-1" />
                                    <span class="text-gray-900 dark:text-white">{{ __('Admit Patient') }}</span>
                                </div>
                            </li>
                        </ol>
                    </nav>
                    ...
                    <div>
                        <h2 class="text-2xl font-bold text-gray-900 dark:text-white tracking-tight leading-7">
                            {{ __('Patient Admission') }}
                        </h2>
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400 max-w-3xl">
                            {{ __('Assign a bed and record admission details for inpatient care.') }}
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div class="p-4 sm:p-6 pb-20">
            <div class="mx-auto max-w-5xl">
                ...
                <div
                    ...
                    {{-- Patient Summary Header (Synced with $admission->patient) --}}
                    <div class="px-6 py-5 bg-slate-50 dark:bg-gray-800/50 border-b border-slate-100 dark:border-gray-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-3 text-sm text-slate-500 dark:text-slate-400 font-mono mt-0.5">
                                    <span>{{ __('UID') }}: {{ $admission->patient->patient_uid }}</span>
                                    <span class="w-1 h-1 bg-slate-400 rounded-full"></span>
                                    <span class="capitalize">{{ $admission->patient->gender }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-amber-50 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 border border-amber-100 dark:border-amber-800 shadow-sm">
                            ...
                            {{ __($admission->status ?? 'Admission Pending') }}
                        </span>
                    </div>
                    ...
                    <div class="p-6 md:p-8">
                        <form wire:submit.prevent="saveAdmission" class="space-y-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {{-- Bed Selection (Synced with $availableBeds) --}}
                                <div class="space-y-1.5">
                                    <label for="bedId" class="block text-sm font-bold text-slate-700 dark:text-slate-300">{{ __('Select Bed') }} <span class="text-red-500">*</span></label>
                                    <select id="bedId" wire:model="bedId"
                                        ...
                                        <option value="">-- {{ __('Choose an Available Bed') }} --</option>
                                        @foreach ($availableBeds as $bed)
                                            <option value="{{ $bed->id }}">
                                                {{ $bed->ward->name ?? 'Ward' }} - {{ $bed->code }} ({{ $bed->bedType->name ?? 'General' }})
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                {{-- Admission Date (Synced with $admissionDate) --}}
                                <div class="space-y-1.5">
                                    <label for="admissionDate" class="block text-sm font-bold text-slate-700 dark:text-slate-300">{{ __('Admission Date') }} <span class="text-red-500">*</span></label>
                                ...
                                {{-- Reason (Synced with $reasonForAdmission) --}}
                                <div class="col-span-1 md:col-span-2 space-y-1.5">
                                    <label for="reasonForAdmission" class="block text-sm font-bold text-slate-700 dark:text-slate-300">{{ __('Reason for Admission') }} <span class="text-red-500">*</span></label>
                                ...
                                {{-- Observation Fee (Synced with $observationFee) --}}
                                <div class="col-span-1 space-y-1.5">
                                    <label for="observationFee" class="block text-sm font-bold text-slate-700 dark:text-slate-300">{{ __('Observation Fee / Deposit') }} <span class="text-slate-400 font-normal ml-1 text-xs">({{ __('Optional') }})</span></label>
