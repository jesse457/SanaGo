                ...
                <div class="flex flex-col gap-2">
                    ...
                    <nav class="flex text-xs font-medium text-gray-500 dark:text-gray-400" aria-label="Breadcrumb">
                        <ol class="inline-flex items-center space-x-1 md:space-x-2">
                            ...
                            <li>
                                <div class="flex items-center">
                                    <x-heroicon-s-chevron-right class="w-3 h-3 text-gray-300 mx-1" />
                                    <span class="text-gray-900 dark:text-white">{{ __('Book Appointment') }}</span>
                                </div>
                            </li>
                        </ol>
                    </nav>
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400 max-w-3xl">
                            {{ __('Search for a patient and assign them to a doctor\'s daily queue.') }}
                        </p>
                    </div>
                </div>
            </div>
                ...
                <form wire:submit="bookAppointment" autocomplete="off">
                    <div class="p-6 md:p-8 space-y-10">
                        ...
                        <section class="space-y-4">
                            <div
                                class="flex items-center justify-between border-b border-slate-100 dark:border-gray-800 pb-3">
                                <h2 class="text-base font-bold text-slate-900 dark:text-white flex items-center gap-3">
                                    <div
                                        class="flex h-7 w-7 items-center justify-center rounded-lg bg-pink-50 text-pink-600 text-sm font-bold dark:bg-blue-900/30 dark:text-blue-400">
                                        1</div>
                                    {{ __('Select Patient') }}
                                </h2>
                                @error('selectedPatientId')
                                    <span
                                        class="text-xs text-red-500 font-bold bg-red-50 dark:bg-red-900/20 px-2 py-1 rounded-md">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="relative">
                                ...
                                @if (!$selectedPatientId)
                                    <div class="relative">
                                        ...
                                        <input type="text" wire:model.live.debounce.300ms="patientSearch"
                                            ...
                                            placeholder="{{ __('Search by name, ID (e.g., P-1234), or phone number...') }}" />
                                    ...
                                    @if (strlen($patientSearch) >= 2)
                                        <div
                                            class="absolute z-20 mt-2 w-full bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-slate-100 dark:border-gray-700 overflow-hidden max-h-80 overflow-y-auto ring-1 ring-black ring-opacity-5">
                                            @forelse ($foundPatients as $patient)
                                                <button type="button"
                                                    ...
                                                    <div>
                                                        <p
                                                            class="font-bold text-slate-900 dark:text-white group-hover:text-blue-700 dark:group-hover:text-blue-400">
                                                            {{ $patient['first_name'] }} {{ $patient['last_name'] }}
                                                        </p>
                                                        <p
                                                            class="text-xs text-slate-500 dark:text-gray-400 mt-0.5 flex items-center gap-2">
                                                            <span
                                                                class="bg-slate-100 dark:bg-gray-700 px-1.5 py-0.5 rounded text-xs font-mono">{{ $patient['patient_uid'] ?? 'N/A' }}</span>
                                                            <span>•</span>
                                                            <span>Ph: {{ $patient['phone'] ?? '--' }}</span>
                                                        </p>
                                                    </div>
                                                    <x-heroicon-s-plus-circle
                                                        class="w-5 h-5 text-slate-300 group-hover:text-blue-500 transition-colors" />
                                                </button>
                                            @empty
                                                <div class="px-4 py-8 text-center">
                                                    <div
                                                        class="w-12 h-12 bg-slate-50 dark:bg-gray-700 rounded-full flex items-center justify-center mx-auto mb-3">
                                                        <x-heroicon-o-user-minus
                                                            class="h-6 w-6 text-slate-400 dark:text-gray-500" />
                                                    </div>
                                                    <p class="text-sm font-medium text-slate-900 dark:text-white">
                                                        {{ __('No patients found') }}</p>
                                                    <p class="text-xs text-slate-500 dark:text-gray-400 mt-1">No matches
                                                        for "{{ $patientSearch }}"</p>
                                                </div>
                                            @endforelse
                                        </div>
                                ...
                                @else
                                    ...
                                    <div
                                        class="relative bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 rounded-xl p-4 flex items-center justify-between animate-fade-in-down">
                                        <div class="flex items-center gap-4">
                                            <div
                                                class="h-12 w-12 rounded-full bg-blue-100 dark:bg-blue-900/50 flex items-center justify-center text-blue-600 dark:text-blue-300 ring-4 ring-white dark:ring-gray-800">
                                                <x-heroicon-s-user class="w-6 h-6" />
                                            </div>
                                            <div>
                                                <p
                                                    class="text-xs font-bold text-blue-600 dark:text-blue-400 uppercase tracking-wider mb-0.5">
                                                    {{ __('Selected Patient') }}</p>
                                                <p class="text-lg font-bold text-slate-900 dark:text-white">
                                                    {{ $selectedPatientName }}</p>
                                            </div>
                                        </div>
                        ...
                        <section class="space-y-4">
                            <div
                                ...
                                <h2 class="text-base font-bold text-slate-900 dark:text-white flex items-center gap-3">
                                    <div
                                        class="flex h-7 w-7 items-center justify-center rounded-lg bg-pink-50 text-pink-600 text-sm font-bold dark:bg-pink-900/30 dark:text-pink-400">
                                        2</div>
                                    {{ __('Select Doctor') }}
                                </h2>
                                @error('doctorId')
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                @forelse ($doctors as $doc)
                                    {{-- Use object syntax $doc->id for Collections --}}
                                    ...
                                    <button type="button" wire:click="$set('doctorId', {{ $doc['id'] }})"
                                        ...
                                        <div class="flex items-start justify-between">
                                            <div class="pr-2">
                                                <p
                                                    class="font-bold text-sm {{ $isSelected ? 'text-white' : 'text-slate-900 dark:text-white' }}">
                                                    {{ $doc['name'] }}
                                                </p>
                                                <p
                                                    class="text-xs mt-1 font-medium {{ $isSelected ? 'text-blue-100' : 'text-slate-500 dark:text-slate-400' }}">
                                            ...
                                            <div
                                                ...
                                                <x-heroicon-s-check
                                                    class="w-3.5 h-3.5 {{ $isSelected ? '' : 'hidden' }}" />
                                            </div>
                                        </div>
                                    </button>
                                @empty
                                    <div
                                        class="col-span-full py-12 text-center border-2 border-dashed border-slate-200 dark:border-gray-700 rounded-xl bg-slate-50 dark:bg-gray-800/50">
                                        <x-heroicon-o-users
                                            class="mx-auto h-10 w-10 text-slate-300 dark:text-gray-600 mb-2" />
                                        <p class="text-sm font-medium text-slate-500 dark:text-gray-400">
                                            {{ __('No doctors available for booking.') }}</p>
                                    </div>
                                @endforelse
                            </div>
                        </section>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="space-y-1.5">
                                    <label for="appointmentDate"
                                        class="block text-sm font-bold text-slate-700 dark:text-slate-300">{{ __('Date') }}</label>
                                    <input type="date" id="appointmentDate" wire:model.live="appointmentDate"
                                ...
                                <div class="space-y-1.5">
                                    <label for="appointmentTime"
                                        class="block text-sm font-bold text-slate-700 dark:text-slate-300">{{ __('Time (Approx)') }}</label>
                                ...
                                <div class="md:col-span-2 space-y-1.5">
                                    <label for="reasonForVisit"
                                        ...
                                        {{ __('Reason for Visit') }} <span
                                            class="text-slate-400 font-normal ml-1 text-xs">({{ __('Optional') }})</span>
                                    </label>
                                    <input type="text" id="reasonForVisit" wire:model="reasonForVisit"
                    ...
                    {{-- Footer / Actions --}}
                    <div
                        class="bg-slate-50 dark:bg-gray-900/50 px-6 py-5 flex items-center justify-between border-t border-slate-100 dark:border-gray-800">
                        {{-- Reset: Navigates to current route to clear all state --}}
