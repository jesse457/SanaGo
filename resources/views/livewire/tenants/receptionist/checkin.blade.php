<main class="w-full min-h-screen bg-slate-50 dark:bg-gray-950 font-sans text-slate-600 dark:text-slate-300">
        ...
        <header
                        ...
                        <ol class="inline-flex items-center space-x-1 md:space-x-2">
                            ...
                            <li>
                                <div class="flex items-center">
                                    ...
                                    <span class="text-gray-900 dark:text-white">Admissions</span>
                                </div>
                            </li>
                        </ol>
                    </nav>
            ...
            <div class="px-4 sm:px-6 py-3 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-700">
                <div class="flex flex-col md:flex-row gap-4 items-center justify-between">
                    ...
                    {{-- Toggle Switch (Show Admitted Only) --}}
                    <div x-data="{ isOpen: false }" class="flex items-center gap-3">
                        <span class="text-sm font-bold text-slate-600 dark:text-slate-400">Show Admitted Only</span>
                        <button @click="isOpen = !isOpen"
                            :class="isOpen ? 'bg-blue-600' : 'bg-slate-200 dark:bg-gray-700'"
                            class="relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
                            <span aria-hidden="true"
                                :class="isOpen ? 'translate-x-5' : 'translate-x-0'"
                                class="pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out"></span>
                        </button>
                        {{-- Wire up the real Livewire property here if it exists, e.g., wire:click="$toggle('showAdmittedOnly')" --}}
                    </div>
                </div>
            </div>
        </header>
        ...
        <div class="relative min-h-[400px] p-4 sm:p-6 pb-20">
            ...
            @if (session()->has('message'))
                <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)"
                    ...
                    class="mb-6 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800 p-4 shadow-sm">
                    <div class="flex">
                        <div class="flex-shrink-0">
                            <x-heroicon-s-check-circle class="h-5 w-5 text-emerald-400" />
                        </div>
                        <div class="ml-3">
                        ...
                        <div class="ml-auto pl-3">
                            <div class="-mx-1.5 -my-1.5">
                                <button @click="show = false" type="button" class="inline-flex rounded-md bg-emerald-50 dark:bg-transparent p-1.5 text-emerald-500 hover:bg-emerald-100 dark:hover:bg-emerald-800 focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:ring-offset-2">
            ...
            {{-- Loading Overlay --}}
            <div wire:loading.flex wire:target="search, admitPatient, viewPatientDetails"
                ...
                <div class="flex items-center gap-3 px-6 py-3 bg-white dark:bg-gray-800 rounded-full shadow-xl border border-slate-100 dark:border-gray-700 animate-bounce">
                    ...
                    <span class="text-sm font-bold text-slate-700 dark:text-slate-200">Processing...</span>
                </div>
            </div>
                    ...
                    <table class="min-w-full divide-y divide-slate-100 dark:divide-gray-800">
                        <thead class="bg-slate-50 dark:bg-gray-950">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Admission Date</th>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @php
                                            ...
                                            $config = match ($admissionStatus) {
                                                'awaiting_bed' => ['bg' => 'bg-amber-50 dark:bg-amber-900/30', 'text' => 'text-amber-700 dark:text-amber-400', 'border' => 'border-amber-100 dark:border-amber-800', 'icon' => 'clock'],
                                                'Discharged' => ['bg' => 'bg-slate-100 dark:bg-slate-800', 'text' => 'text-slate-600 dark:text-slate-400', 'border' => 'border-slate-200 dark:border-slate-700', 'icon' => 'arrow-right-on-rectangle'],
                                            ...
                                            $statusLabel = match ($admissionStatus) {
                                                ...
                                                'Admitted' => 'Admitted' . ($latestAdmission?->bed_id ? " (Bed {$latestAdmission->bed_id})" : ''),
                                        ...
                                        <div class="flex items-center justify-center gap-2">
                                            @if (($latestAdmission?->status ?? '') === 'Admitted')
                                                <span class="inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-bold bg-slate-100 text-slate-500 dark:bg-gray-800 dark:text-gray-500 cursor-not-allowed border border-transparent">
                                                    <x-heroicon-s-check class="w-3.5 h-3.5 mr-1" /> Active
                                                </span>
                                            @else
                ...
                @forelse ($patients as $patient)
                    <div wire:key="mobile-card-{{ $patient->id }}" class="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-slate-200 dark:border-gray-800 p-4">
                        <div class="flex items-start justify-between mb-3">
                            ...
                            @php
                                $latestAdmission = $patient->admissions->first();
                                $statusLabel = $latestAdmission?->status === 'awaiting_bed' ? 'Awaiting' : ($latestAdmission?->status ?? 'None');
                                $statusClass = match($latestAdmission?->status ?? '') {
                                    'awaiting_bed' => 'bg-amber-50 text-amber-700 border-amber-100',
                        ...
                        <div class="grid grid-cols-2 gap-2 text-xs mb-3">
                            <div class="bg-slate-50 dark:bg-gray-800/50 p-2 rounded-lg">
                                ...
                                <span class="font-medium text-slate-700 dark:text-slate-300">{{ $patient->admissions->first()?->doctor->name ?? '--' }}</span>
                            </div>
                            <div class="bg-slate-50 dark:bg-gray-800/50 p-2 rounded-lg">
                                <span class="block text-slate-400 uppercase text-[10px] font-bold">Admit Date</span>
                                <span class="font-medium text-slate-700 dark:text-slate-300">
                                    {{ $latestAdmission?->admission_date?->format('M d') ?? '--' }}
                                </span>
                            </div>
                        </div>

                        <div class="flex justify-end gap-2 border-t border-slate-100 dark:border-gray-800 pt-3">
                            @if (($latestAdmission?->status ?? '') !== 'Admitted')
                                <button wire:click="admitPatient({{ $patient->id }})" class="p-2 text-blue-600 bg-blue-50 rounded-lg">
                                    <x-heroicon-o-plus class="w-5 h-5" />
                                </button>
                            @endif
                            <button wire:click="viewPatientDetails({{ $patient->id }})" class="p-2 text-slate-600 bg-slate-50 rounded-lg border border-slate-100">
    ...
    <div x-data="{ show: @entangle('showAdmissionModal') }" x-show="show" x-cloak class="relative z-50" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        {{-- Backdrop --}}
                    ...
                    <div class="px-6 py-6 bg-slate-50 dark:bg-gray-900/50 max-h-[60vh] overflow-y-auto custom-scrollbar">
                        @if ($recentAdmissions->isEmpty())
                            <div class="text-center py-8">
                                <div class="h-14 w-14 rounded-full bg-slate-100 dark:bg-gray-800 mx-auto flex items-center justify-center mb-3">
                                    <x-heroicon-o-clipboard-document class="w-7 h-7 text-slate-400" />
                                </div>
                                <h3 class="text-base font-bold text-slate-900 dark:text-white">No pending requests</h3>
                                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">There are no recent admission requests for this patient.</p>
                                <div class="mt-6 flex justify-center gap-3">
                                    <button wire:click="confirmAdmission(null)"
                                        class="inline-flex items-center px-5 py-2.5 rounded-xl bg-blue-600 text-white text-sm font-bold shadow-md hover:bg-blue-700 transition-all">
                                        Create New Admission
                                    </button>
                                    <button wire:click="$set('showAdmissionModal', false)"
                                        class="inline-flex items-center px-5 py-2.5 rounded-xl bg-white dark:bg-gray-800 text-slate-700 dark:text-slate-300 border border-slate-300 dark:border-gray-600 text-sm font-bold hover:bg-slate-50 dark:hover:bg-gray-700 transition-all">
                                        Close
                                    </button>
                                </div>
                            </div>
                        @else
                                    ...
                                    <div wire:click="selectAdmission({{ $ad->id }})"
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <div class="flex justify-between items-start mb-1">
                                                ...
                                                <span class="inline-flex items-center rounded-lg px-2 py-1 text-xs font-bold ring-1 ring-inset {{ $ad->status === 'Admitted' ? 'bg-emerald-50 text-emerald-700 ring-emerald-600/20' : 'bg-amber-50 text-amber-800 ring-amber-600/20' }}">
                                                    {{ $ad->status }}
                                                </span>
                                            </div>
                                            <p class="text-sm text-slate-600 dark:text-slate-300 mb-1">
                                                <span class="font-bold text-slate-700 dark:text-slate-200">Requested by:</span> {{ optional($ad->doctor)->name ?? 'Unknown' }}
                                            </p>
                                            <p class="text-sm text-slate-500 dark:text-slate-400 line-clamp-2 bg-slate-50 dark:bg-gray-800/50 p-2 rounded-lg border border-slate-100 dark:border-gray-700 italic">
                            ...
                            <div class="mt-6 flex items-center justify-end gap-3 pt-4 border-t border-slate-100 dark:border-gray-800">
                                <button type="button" wire:click="$set('showAdmissionModal', false)"
                                    class="px-4 py-2.5 rounded-xl text-sm font-bold text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-gray-700 transition-colors">
                                    Cancel
                                </button>
