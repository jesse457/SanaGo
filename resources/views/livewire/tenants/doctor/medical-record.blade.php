<div class="bg-gray-50 dark:bg-gray-900 overflow-hidden h-screen flex flex-col" x-data="{ activeTab: 'clinical' }">
    ...
    <header class="flex-shrink-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm z-20">
        <div class="px-6 py-4 flex items-center justify-between">
            <div>
                ...
                <h1 class="text-xl font-bold text-gray-900 dark:text-white tracking-tight">Patient Consultation</h1>
            </div>
        </div>
    </header>
        ...
        <aside class="w-full md:w-80 lg:w-96 flex-shrink-0 flex flex-col border-r border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-800 z-10 h-full">
            <div class="p-4 border-b border-gray-100 dark:border-gray-700 relative z-20">
                ...
                <div class="relative">
                    ...
                    <input type="text" wire:model.live.debounce.300ms="patientQuery" class="w-full pl-10 pr-10 py-2 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-blue-500" placeholder="Name or ID...">
                    ...
                    @if(strlen($patientQuery) >= 2)
                        <div class="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden z-50">
                            @forelse($patientResults as $res)
                                <button wire:click="selectPatient({{ $res->id }})" class="w-full text-left px-4 py-3 hover:bg-blue-50 dark:hover:bg-gray-700 border-b border-gray-50 dark:border-gray-700 flex items-center gap-3 transition-colors">
                            ...
                            @empty
                                <div class="px-4 py-8 text-center text-gray-500">No patient found</div>
                            @endforelse
                        </div>
                    @endif
                </div>
                ...
                <div class="flex-1 overflow-y-auto p-5 custom-scrollbar">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="bg-red-50 dark:bg-red-900/10 p-3 rounded-lg border border-red-100 dark:border-red-800/30">
                            <span class="text-xs font-semibold text-red-400 uppercase flex items-center gap-1"><x-heroicon-s-exclamation-triangle class="w-3 h-3"/> Allergies</span>
                            <p class="text-sm font-medium text-red-800 dark:text-red-300 mt-0.5">{{ $patient->allergies ?? 'No known allergies' }}</p>
                        </div>
                    </div>
                </div>
            @else
                <div class="flex-1 flex flex-col items-center justify-center text-gray-400 p-6 text-center">
                    <x-heroicon-o-user class="w-16 h-16 mb-2 opacity-50"/><p class="text-sm">Select a patient to begin</p>
                </div>
            @endif
        </aside>
                ...
                <div class="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 pt-4 flex items-center justify-between flex-shrink-0 z-10">
                    <div class="flex space-x-6 overflow-x-auto no-scrollbar">
                        @foreach(['clinical' => 'Clinical Notes', 'rx' => 'Prescriptions', 'labs' => 'Labs & Tests', 'files' => 'Attachments'] as $key => $label)
                            <button @click="activeTab = '{{ $key }}'" :class="activeTab === '{{ $key }}' ? 'border-blue-600 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400 hover:border-gray-300'" class="whitespace-nowrap pb-4 px-1 border-b-2 font-medium text-sm transition-colors flex items-center gap-2 outline-none">
                                 {{ $label }}
                    ...
                    <div class="flex items-center gap-2 pb-2 pl-4">
                        <span wire:loading wire:target="saveDraft, saveAndSign" class="text-xs text-gray-500 animate-pulse">Saving...</span>
                        <button wire:click="saveDraft" wire:loading.attr="disabled" class="px-3 py-1.5 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 rounded-lg text-sm font-medium hover:bg-gray-50">Draft</button>
                        <button wire:click="saveAndSign" wire:loading.attr="disabled" wire:confirm="Finalize this record?" class="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium flex items-center gap-2">
                ...
                <div class="flex-1 overflow-y-auto p-6 custom-scrollbar">
                    ...
                    <div x-show="activeTab === 'clinical'" class="space-y-6 max-w-4xl mx-auto" x-transition.opacity>
                        <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
                            ...
                            <textarea wire:model.blur="complaint" rows="3" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-900 focus:ring-blue-500" placeholder="Reason for visit..."></textarea>
                        ...
                        <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
                            ...
                            <textarea wire:model.blur="clinicalNotes" rows="6" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-900 focus:ring-blue-500" placeholder="Detailed notes..."></textarea>
                        </div>
                        <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
                            <label class="block text-sm font-bold text-gray-700 dark:text-gray-200 mb-2">Diagnosis</label>
                            <textarea wire:model.blur="diagnosisText" rows="2" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-900 focus:ring-blue-500" placeholder="Final diagnosis..."></textarea>
                        </div>
                    </div>
                    ...
                    <div x-show="activeTab === 'rx'" class="space-y-6 max-w-5xl mx-auto" x-cloak>
                        <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
                            <div class="mb-6" x-data="{ tempMedId: '' }">
                                <label class="block text-xs font-semibold text-gray-500 uppercase mb-1">Add Medication</label>
                                <div class="flex gap-2">
                                    <select x-model="tempMedId" class="flex-1 py-2 bg-white dark:bg-gray-900 border border-gray-300 rounded-lg text-sm">
                                        <option value="">Select a medication...</option>
                                        @foreach($allMedications as $med)
                                            <option value="{{ $med->id }}">{{ $med->name }} (Stock: {{ $med->stock_quantity }})</option>
                                        @endforeach
                                    </select>
                                    <button @click="$wire.addMedication(tempMedId); tempMedId = ''" :disabled="!tempMedId" class="px-4 py-2 bg-blue-600 text-white rounded-lg disabled:bg-gray-400 flex items-center gap-2"><x-heroicon-m-plus class="w-5 h-5"/> Add</button>
                                </div>
                            </div>
                                        ...
                                        <tbody class="bg-white divide-y divide-gray-200">
                                            @foreach($prescriptionItems as $index => $item)
                                                <tr wire:key="rx-item-{{ $index }}">
                                                    ...
                                                    <td class="px-4 py-2"><input type="text" wire:model="prescriptionItems.{{ $index }}.dosage" class="w-full text-sm border-gray-200 rounded p-1"></td>
                                                    <td class="px-4 py-2"><input type="text" wire:model="prescriptionItems.{{ $index }}.frequency" class="w-full text-sm border-gray-200 rounded p-1"></td>
                                                    <td class="px-4 py-2"><input type="text" wire:model="prescriptionItems.{{ $index }}.duration" class="w-full text-sm border-gray-200 rounded p-1"></td>
                                                    <td class="px-4 py-2"><button wire:click="removeMedication({{ $index }})" class="text-red-500"><x-heroicon-m-trash class="w-4 h-4"/></button></td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
                            ...
                            @else
                                <div class="text-center py-8 text-gray-500 bg-gray-50 rounded-lg border border-dashed">No medications added.</div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div x-show="activeTab === 'labs'" class="space-y-6 max-w-5xl mx-auto" x-cloak>
                        <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
                             <div class="mb-6" x-data="{ tempLabId: '' }">
                                <label class="block text-xs font-semibold text-gray-500 uppercase mb-1">Add Lab Test</label>
                                <div class="flex gap-2">
                                    <select x-model="tempLabId" class="flex-1 py-2 bg-white dark:bg-gray-900 border border-gray-300 rounded-lg text-sm">
                                        <option value="">Select a lab test...</option>
                                        @foreach($allLabTests as $test)
                                            <option value="{{ $test->id }}">{{ $test->test_name }} ({{ $test->code }})</option>
                                        @endforeach
                                    </select>
                                    <button @click="$wire.addLabTest(tempLabId); tempLabId = ''" :disabled="!tempLabId" class="px-4 py-2 bg-purple-600 text-white rounded-lg disabled:bg-gray-400 flex items-center gap-2"><x-heroicon-m-plus class="w-5 h-5"/> Add</button>
                                </div>
                            </div>
                            ...
                            @if(count($labItems) > 0)
                                <div class="space-y-3">
                                    @foreach($labItems as $index => $item)
                                        <div wire:key="lab-item-{{ $index }}" class="p-4 rounded-lg border border-gray-200 bg-white flex flex-col md:flex-row gap-4 items-center">
                                            <div class="w-full md:w-1/4">
                                                <div class="font-bold text-gray-900">{{ $item['test_name'] }}</div>
                                                <select wire:model="labItems.{{ $index }}.urgency" class="mt-1 text-xs border-gray-200 rounded-md">
                                            ...
                                            <button wire:click="removeLabTest({{ $index }})" class="text-red-500"><x-heroicon-m-x-mark class="w-5 h-5"/></button>
                                        </div>
                                    @endforeach
                                </div>
                            @else
                                <div class="text-center py-8 text-gray-500 bg-gray-50 rounded-lg border border-dashed">No lab tests requested.</div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div x-show="activeTab === 'files'" class="space-y-6 max-w-5xl mx-auto" x-cloak>
                        <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
                            <div class="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center relative">
                                ...
                                <h3 class="font-medium">Click or Drag files here</h3>
                            ...
                            @if($attachments)
                                ...
                                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    @foreach($attachments as $index => $file)
                                        <div wire:key="tmp-file-{{ $index }}" class="relative bg-gray-50 border rounded-lg p-2 group">
                                            @if(str_contains($file->getMimeType(), 'image'))
                                                <img src="{{ $file->temporaryUrl() }}" class="h-24 w-full object-cover rounded">
                                            @else
                                                <div class="h-24 w-full flex items-center justify-center bg-gray-200 rounded"><x-heroicon-o-document class="w-10 h-10"/></div>
                                            @endif
                                            <button wire:click="removeTempAttachment({{ $index }})" class="absolute top-1 right-1 bg-red-500 text-white rounded-full p-1 opacity-0 group-hover:opacity-100"><x-heroicon-s-x-mark class="w-3 h-3"/></button>
                                        </div>
                                    @endforeach
                                </div>
                            @endif
                            ...
                            @if(count($storedAttachments) > 0)
                                ...
                                <div class="space-y-2">
                                    @foreach($storedAttachments as $att)
                                        <div wire:key="stored-file-{{ $att->id }}" class="flex items-center justify-between p-3 border rounded-lg">
                                            <div class="flex items-center gap-3">
                                                <div class="h-10 w-10 bg-blue-50 text-blue-600 rounded-lg flex items-center justify-center"><x-heroicon-o-paper-clip class="w-5 h-5"/></div>
                                                <div>
                                                    <a href="{{ $attachmentUrls[$att->id] ?? '#' }}" target="_blank" class="text-sm font-medium text-blue-600 underline">{{ $att->file_name }}</a>
                                                    <p class="text-xs text-gray-500">{{ $att->created_at->format('M d, Y H:i') }}</p>
                                                </div>
                                            </div>
                                            <button wire:click="removeStoredAttachment({{ $att->id }})" wire:confirm="Delete file?" class="text-gray-400 hover:text-red-500"><x-heroicon-m-trash class="w-4 h-4"/></button>
                                        </div>
                                    @endforeach
                                </div>
                            @endif
            ...
            @else
                <div class="absolute inset-0 flex items-center justify-center">
                    <div class="flex flex-col items-center text-gray-400">
                        <x-heroicon-o-user-plus class="w-16 h-16 mb-4 opacity-20"/>
                        <h3 class="text-lg font-medium">Ready for Consultation</h3>
                        <p class="text-sm">Search for a patient on the left to start.</p>
                    </div>
                </div>
            @endif
        </main>
