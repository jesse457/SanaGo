<div class="bg-gray-50 dark:bg-gray-900 overflow-hidden"
 ...
 {{-- 🟢 LEFT SIDEBAR: Patient Context & Selection --}}
    <aside class="h-[calc(100vh-theme(spacing.16))]   w-full md:w-80 lg:w-96 flex-shrink-0 flex flex-col border-r border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-800 z-10">
                    ...
                    <div class="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden z-50">
                        @foreach($patientResults as $res)
                            <button wire:click="selectPatient({{ $res->id }})"
                                ...
                                <div>
                                    <div class="font-bold text-gray-900 dark:text-white text-sm">{{ $res->first_name }} {{ $res->last_name }}</div>
        ...
        @if($patient)
            <div class="flex-1 overflow-y-auto p-5 custom-scrollbar">
                ...
                <div class="space-y-4">
                    ...
                    <!-- Placeholder for Vitals (could be a component) -->
                    <div class="grid grid-cols-2 gap-3">
                        <div class="bg-white dark:bg-gray-700 p-2 rounded border border-gray-200 dark:border-gray-600">
                            <span class="text-[10px] text-gray-400 uppercase">BP</span>
                            <div class="font-bold text-gray-800 dark:text-white">120/80</div>
                        </div>
                         <div class="bg-white dark:bg-gray-700 p-2 rounded border border-gray-200 dark:border-gray-600">
                            <span class="text-[10px] text-gray-400 uppercase">Weight</span>
                            <div class="font-bold text-gray-800 dark:text-white">72 kg</div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    {{-- 🔵 RIGHT MAIN: Consultation Worksheet --}}
    <main class="flex-1 flex flex-col h-full relative min-w-0 bg-gray-50 dark:bg-gray-900">
        @if($selectedPatientId)
            {{-- Tabs Header --}}
            <div class="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 pt-4 flex items-center justify-between flex-shrink-0">
                ...
                <div class="flex items-center gap-2 pb-2">
                    <span wire:loading class="text-xs text-gray-500 animate-pulse mr-2">Saving...</span>

                    <button wire:click="saveDraft" wire:loading.attr="disabled"
                            class="px-4 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 rounded-lg text-sm font-medium hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors">
                        Save Draft
                    </button>
                    <button wire:click="saveAndSign" wire:loading.attr="disabled"
                            wire:confirm="Are you sure you want to finalize this record? It cannot be edited afterwards."
                            class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium shadow-sm transition-colors flex items-center gap-2">
                        <x-heroicon-m-check-badge class="w-4 h-4"/> Sign & Close
                    </button>
                </div>
            </div>
            ...
            <div class="flex-1 overflow-y-auto p-6 custom-scrollbar">
                ...
                <div x-show="activeTab === 'clinical'" class="space-y-6 max-w-4xl mx-auto" x-transition.opacity>
                    <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
                        ...
                        <textarea wire:model.blur="complaint" rows="3" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-900 focus:ring-blue-500 focus:border-blue-500" placeholder="Patient's primary reason for visit..."></textarea>
                    ...
                    <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
                        ...
                        <textarea wire:model.blur="clinicalNotes" rows="6" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-900 focus:ring-blue-500 focus:border-blue-500" placeholder="Detailed examination notes..."></textarea>
                    </div>

                    <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
                        <label class="block text-sm font-bold text-gray-700 dark:text-gray-200 mb-2">Diagnosis</label>
                        <textarea wire:model.blur="diagnosisText" rows="2" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-900 focus:ring-blue-500 focus:border-blue-500" placeholder="Provisional or final diagnosis..."></textarea>
                    </div>
                </div>
                ...
                <div x-show="activeTab === 'rx'" class="space-y-6 max-w-5xl mx-auto" x-cloak>
                    <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
                        <div class="flex gap-4 items-end mb-6">
                            <div class="flex-1">
                                <label class="block text-xs font-semibold text-gray-500 uppercase mb-1">Select Medication</label>
                                <select wire:model="selectedMedicationId" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-900">
                                    <option value="">-- Choose Drug --</option>
                                    @foreach($medicationOptions as $med)
                                        <option value="{{ $med->id }}">{{ $med->name }} ({{ $med->stock_quantity }} in stock)</option>
                                    @endforeach
                                </select>
                            </div>
                            <button wire:click="addMedication" class="px-4 py-2.5 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-lg font-medium transition-colors">
                                    ...
                                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                        @foreach($prescriptionItems as $index => $item)
                                            <tr>
                                                ...
                                                <td class="px-4 py-2"><input type="text" wire:model="prescriptionItems.{{ $index }}.dosage" class="w-full text-sm border-gray-200 rounded p-1" placeholder="e.g. 500mg"></td>
                                                <td class="px-4 py-2"><input type="text" wire:model="prescriptionItems.{{ $index }}.frequency" class="w-full text-sm border-gray-200 rounded p-1" placeholder="e.g. 1-0-1"></td>
                                                <td class="px-4 py-2"><input type="text" wire:model="prescriptionItems.{{ $index }}.duration" class="w-full text-sm border-gray-200 rounded p-1" placeholder="e.g. 5 days"></td>
                        ...
                        @else
                            <div class="text-center py-10 bg-gray-50 dark:bg-gray-900 rounded-lg border border-dashed border-gray-300">
                ...
                <div x-show="activeTab === 'labs'" class="space-y-6 max-w-5xl mx-auto" x-cloak>
                    <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
                        <div class="flex gap-4 items-end mb-6">
                            <div class="flex-1">
                                <label class="block text-xs font-semibold text-gray-500 uppercase mb-1">Select Lab Test</label>
                                <select wire:model="selectedLabTestId" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-900">
                                    <option value="">-- Choose Test --</option>
                                    @foreach($labTestOptions as $test)
                                        <option value="{{ $test->id }}">{{ $test->test_name }} ({{ $test->code }})</option>
                                    @endforeach
                                </select>
                            </div>
                            <button wire:click="addLabTest" class="px-4 py-2.5 bg-purple-50 text-purple-600 hover:bg-purple-100 rounded-lg font-medium transition-colors">
                                <x-heroicon-m-plus class="w-5 h-5"/>
                            </button>
                        </div>
                        ...
                        @if(count($labItems) > 0)
                            <div class="space-y-3">
                                @foreach($labItems as $index => $item)
                                    <div class="p-4 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex flex-col md:flex-row gap-4 items-start md:items-center">
                                        ...
                                        <div class="flex-1 w-full grid grid-cols-1 md:grid-cols-2 gap-3">
                                            ...
                                            <div>
                                                ...
                                                <input type="text" wire:model="labItems.{{ $index }}.reason" class="w-full text-sm border-gray-200 rounded p-1.5" placeholder="Reason for test...">
                                            </div>
                                        </div>
                ...
                {{-- 4. FILES TAB (FIXED MULTI UPLOAD) --}}
                            ...
                            <div class="space-y-2">
                                @foreach($storedAttachments as $att)
                                    <div class="flex items-center justify-between p-3 bg-white border border-gray-200 rounded-lg hover:shadow-sm transition-shadow">
                                        ...
                                        <button wire:click="removeStoredAttachment({{ $att->id }})" wire:confirm="Delete this file?" class="text-gray-400 hover:text-red-500 p-2">
        ...
        @else
            ...
            <div class="flex-1 flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-900 text-gray-400">
                ...
                <p class="max-w-xs text-center mt-2">Search for a patient on the left to view their history and start a new medical record.</p>
            </div>
        @endif
    </main>
    </div>
