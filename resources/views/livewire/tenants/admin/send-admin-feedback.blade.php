<div class="flex-1 p-4  bg-gray-100 dark:bg-gray-900 overflow-y-auto min-h-screen">
    <header class="mb-8 space-y-4 sm:space-y-0">
        <nav class="flex items-center text-sm font-medium text-gray-500 dark:text-gray-400">
            ...
            <x-heroicon-s-chevron-right class="w-4 h-4 mx-1" />
            <span class="text-gray-400 ">{{ __('admin.submit_feedback_breadcrumb') }}</span>
        </nav>
    ...
    <div
        ...
        <div class="px-6 py-5 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
            <div>
                <h2 class="text-xl font-semibold text-gray-900 dark:text-gray-100">{{ __('admin.form_header_title') }}</h2>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('admin.form_header_description') }}</p>
            </div>
            <span
                class="inline-flex items-center rounded-full bg-gray-200 dark:bg-gray-700 px-3 py-1 text-xs font-medium text-gray-700 dark:text-gray-300">{{ __('admin.form_header_app_name') }}</span>
        </div>
        ...
        <form wire:submit="submit" class="p-6 space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="subject" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('admin.label_subject') }}</label>
                    <input id="subject" type="text" wire:model.defer="subject"
                        placeholder="{{ __('admin.placeholder_subject') }}" class="form-input" />
                    @error('subject')
                ...
                <div>
                    <label for="category" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('admin.label_category') }}</label>
            ...
            <div>
                <label for="message" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('admin.label_message') }}</label>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pt-4">
                <div class="text-xs text-gray-500 dark:text-gray-400">
                    {{ __('admin.disclaimer_prefix') }} <a href="#"
                        class="text-indigo-600 hover:underline">{{ __('admin.disclaimer_rules') }}</a> {{ __('admin.disclaimer_and') }} <a href="#"
                        class="text-indigo-600 hover:underline">{{ __('admin.disclaimer_privacy_policy') }}</a>.
                </div>
