            ...
            <div class="px-4 sm:px-6 py-4 md:flex md:items-center md:justify-between space-y-3 md:space-y-0">
                ...
                <div class="flex-1 min-w-0">
                        ...
                        <ol class="inline-flex items-center space-x-1 md:space-x-2">
                            ...
                            <li>
                                <div class="flex items-center">
                                    ...
                                    <span class="text-gray-900 dark:text-white">{{ __('admin.feedbacks_bar') }}</span>
                                </div>
                            </li>
                        </ol>
                    </nav>
                    ...
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        {{ __('admin.feedback_history_description') }}
                    </p>
                </div>

                {{-- Action Toolbar --}}
                <div class="flex items-center gap-3">
                    <a href="{{ route('admin.admin-feedback') }}" wire:navigate
                        ...
                        <span>{{ __('admin.feedback_button_submit') }}</span>
                    </a>
                </div>
            </div>
        </header>
        ...
        <div class="relative min-h-[400px] p-4 sm:p-6 pb-20">
            ...
            <div wire:loading.flex
                ...
                <div
                    ...
                    <span class="text-sm font-bold text-slate-700 dark:text-slate-200">Loading feedbacks...</span>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-slate-200 dark:border-gray-800 overflow-hidden">
                @if (!isset($feedbacks) || $feedbacks->total() === 0)
                    <div class="text-center py-16 px-6">
                        <div class="w-20 h-20 bg-slate-50 dark:bg-gray-800 rounded-full flex items-center justify-center mx-auto mb-6 border border-slate-100 dark:border-gray-700">
                            <x-heroicon-o-chat-bubble-left-right class="h-10 w-10 text-slate-400" />
                        </div>
                        <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-2">
                            {{ __('admin.feedback_no_feedback_title') }}
                        </h3>
                        <p class="text-slate-500 dark:text-slate-400 max-w-sm mx-auto mb-8">
                            {{ __('admin.feedback_no_feedback_message') }}
                        </p>
                        <button wire:click="goToSendFeedback"
                            ...
                            <x-heroicon-o-plus class="w-5 h-5" />
                            {{ __('admin.feedback_button_submit') }}
                        </button>
                    </div>
                @else
                            ...
                            <thead class="bg-slate-50 dark:bg-gray-950">
                                <tr>
                                    <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">
                                        {{ __('admin.feedback_table_subject') }}
                                    </th>
                                    <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider hidden sm:table-cell">
                                        {{ __('admin.feedback_table_category') }}
                                    </th>
                                    <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">
                                        {{ __('admin.feedback_table_status') }}
                                    </th>
                                    <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider hidden md:table-cell">
                                        {{ __('admin.feedback_table_submitted') }}
                                    </th>
                                ...
                                @foreach ($feedbacks as $feedback)
                                    @php
                                        ...
                                        $statusStyles = match($status) {
                                            ...
                                            'closed' => 'bg-slate-50 text-slate-700 border-slate-100 dark:bg-gray-800 dark:text-slate-300 dark:border-gray-700',
                                            default => 'bg-purple-50 text-purple-700 border-purple-100 dark:bg-purple-900/30 dark:text-purple-300 dark:border-purple-800',
                                        };
                                    @endphp

                                    <tr class="group hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors duration-150">
                                        <td class="px-6 py-4">
                                            <div class="text-sm font-bold text-slate-900 dark:text-white mb-0.5">
                                                {{ $feedback->subject ?? __('admin.feedback_no_subject') }}
                                            </div>
                                            <div class="text-xs text-slate-500 dark:text-slate-400 line-clamp-1 max-w-xs">
                                                {{ Str::limit($feedback->message ?? '', 80) }}
                                            </div>
                                        </td>

                                        <td class="px-6 py-4 whitespace-nowrap hidden sm:table-cell">
                                            <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium bg-slate-100 text-slate-700 border border-slate-200 dark:bg-gray-800 dark:text-slate-300 dark:border-gray-700 capitalize">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold border shadow-sm capitalize {{ $statusStyles }}">
                                                {{ ucfirst($status !== 'unknown' ? $status : __('admin.feedback_status_unknown')) }}
                                            </span>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap hidden md:table-cell text-sm text-slate-500 dark:text-slate-400">
                                            ...
                                            <span class="text-xs text-slate-400 ml-1">{{ optional($feedback->created_at)->format('H:i') }}</span>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <button wire:click.prevent="showFeedback({{ $feedback->id }})"
                                                ...
                                                <x-heroicon-s-eye class="w-4 h-4" /> {{ __('admin.feedback_action_view') }}
                                            </button>
                                        </td>
                                    </tr>
                                @endforeach
                        ...
                        <div x-show="open"
                            ...
                            <div class="bg-white dark:bg-gray-900 px-6 py-5 border-b border-slate-100 dark:border-gray-800 flex items-center justify-between sticky top-0 z-10">
                                <h3 class="text-xl font-bold text-slate-900 dark:text-white">
                                    {{ $modalTitle ?? __('admin.feedback_details_title') }}
                                </h3>
                                    ...
                                    <div class="space-y-8">
                                        ...
                                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-50 dark:bg-gray-800/50 p-4 rounded-xl border border-slate-100 dark:border-gray-800">
                                            <div class="flex flex-wrap items-center gap-3">
                                                ...
                                                <span class="text-xs font-medium text-slate-500 dark:text-slate-400 flex items-center gap-1">
                                                    ...
                                                    {{ optional($modalFeedback->created_at)->format('M d, Y • h:i A') }}
                                                </span>
                                            </div>

                                            <div class="flex items-center gap-2">
                                                <span class="text-xs font-bold text-slate-400 uppercase tracking-wider">{{ __('admin.modal_label_status') }}:</span>
                                        ...
                                        <div class="relative">
                                            ...
                                            <div class="relative flex justify-center">
                                                <span class="bg-white dark:bg-gray-900 px-3 text-xs font-bold text-slate-400 uppercase tracking-wider">
                                                    {{ __('admin.modal_section_response') }}
                                                </span>
                                            </div>
                                        </div>
                                                ...
                                                <div class="flex-grow">
                                                    <div class="bg-blue-50/50 dark:bg-blue-900/10 rounded-2xl rounded-tl-none p-5 border border-blue-100 dark:border-blue-900/30">
                                                        ...
                                                        <div class="flex items-center gap-2 text-xs text-slate-400 font-medium">
                                                            <span>{{ __('admin.feedback_responded_by') }} <span class="text-slate-600 dark:text-slate-300 font-bold">{{ optional($modalFeedback->responder)->name ?? __('admin.feedback_default_staff') }}</span></span>
                                                            <span>•</span>
                                        ...
                                        @else
                                            <div class="text-center py-8 bg-slate-50 dark:bg-gray-800/50 rounded-2xl border border-dashed border-slate-300 dark:border-gray-700">
                                                ...
                                                <p class="text-sm font-medium text-slate-500 dark:text-slate-400">{{ __('admin.modal_no_response_yet') }}</p>
                                            </div>
                                        @endif
                                    </div>
                                @else
