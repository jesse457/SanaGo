<main class="flex-1 p-4  bg-gray-50 dark:bg-gray-900 min-h-screen">
    <div class="mb-6">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <span class="ms-1 text-sm font-medium text-gray-700 md:ms-2 dark:text-gray-300">{{ __('admin.feedbacks_bar') }}
                        </span>
                    </div>
                </li>
            </ol>
    ...
    <header class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="text-3xl font-bold text-gray-800 dark:text-white mb-1 flex items-center gap-3">
                <x-heroicon-s-user-group class="w-8 h-8 text-indigo-600" />
                {{ __('admin.feedbacks_bar') }}
            </h1>
            <p class="text-gray-600 dark:text-gray-400">{{ __('admin.feedback_history_description') }}</p>
        </div>
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.admin-feedback') }}" wire:navigate
                ...
                <x-heroicon-o-plus class="w-5 h-5" /> {{ __('admin.feedback_button_submit') }}
            </a>
        </div>
    </header>
    ...
    <div class="bg-white rounded-xl shadow-lg border border-gray-100 p-6 mb-10 dark:bg-gray-800 dark:border-gray-700">
        ...
        @if (!isset($feedbacks) || $feedbacks->total() === 0)
            <div class="text-center py-12 text-gray-500 dark:text-gray-300">
                ...
                <p class="text-lg font-medium">{{ __('admin.feedback_no_feedback_title') }}</p>
                <p class="mt-2 text-sm">{{ __('admin.feedback_no_feedback_message') }}</p>
                <div class="mt-4">
        ...
        @else
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900 dark:text-white">
                                        {{ $feedback->subject ?? __('admin.feedback_no_subject') }}</div>
                                ...
                                <td class="px-4 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end gap-2">
                                        <button wire:click.prevent="showFeedback({{ $feedback->id }})"
                                            ...
                                            <x-heroicon-s-eye class="w-4 h-4 text-gray-600 dark:text-gray-200" /> {{ __('admin.feedback_action_view') }}
                                        </button>
            ...
            <div class="mt-4 flex items-center justify-between gap-4">
                <div class="text-sm text-gray-600 dark:text-gray-300">
                    {{ __('admin.feedback_pagination_showing') }} <span class="font-semibold">{{ $feedbacks->firstItem() ?? 0 }}</span> {{ __('admin.feedback_pagination_to') }} <span
                        class="font-semibold">{{ $feedbacks->lastItem() ?? 0 }}</span> {{ __('admin.feedback_pagination_of') }} <span
               ...
               <div role="dialog" aria-modal="true" aria-labelledby="modal-title"
                   ...
                   <div class="px-6 py-4 border-b dark:border-gray-700 flex justify-between items-center">
                       <h3 id="modal-title" class="text-lg font-semibold text-gray-900 dark:text-white">
                           {{ $modalTitle ?? __('admin.feedback_details_title') }}</h3>
        ...
        <div class="space-y-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                <div class="min-w-0">
                    ...
                    <div class="mt-2 text-sm text-gray-500 dark:text-gray-400 flex flex-wrap items-center gap-2">
                        ...
                        <span class="text-xs text-gray-400">
                            {{ __('admin.feedback_table_submitted') }} {{ optional($modalFeedback->created_at)->diffForHumans() }}
                        </span>
                ...
                <div class="text-right hidden sm:block">
                    <div class="text-xs text-gray-500 dark:text-gray-300">{{ __('admin.modal_label_status') }}</div>
                    <div class="mt-1">
                        <span
            ...
            {{-- Published response --}}
            <div>
                <h5 class="text-sm font-medium text-gray-800 dark:text-gray-100">{{ __('admin.modal_section_response') }}</h5>
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div class="min-w-0">
                                ...
                                <div class="text-xs text-gray-400 mt-3">
                                    {{ __('admin.feedback_responded_by') }} {{ optional($modalFeedback->responder)->name ?? __('admin.feedback_default_staff') }} •
                ...
                @else
                    <div
                        ...
                        <div class="font-medium">{{ __('admin.modal_no_response_yet') }}</div>
                    </div>
                @endif
            </div>
    ...
    @else
        <div class="text-center py-6 text-gray-500 dark:text-gray-300">{{ __('admin.activities_loading') }}</div>
        @endif
    </div>
    </div>
    </div>
