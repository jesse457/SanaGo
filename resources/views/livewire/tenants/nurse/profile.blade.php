...
<div class="mb-8">
                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <span class="ms-1 text-sm text-gray-400 md:ms-2 dark:text-gray-200">Profile</span>
                        </div>
                    </li>
                </ol>
            </nav>
                        ...
                        <div class="flex justify-end">
                            <button type="submit"
                                ...
                                <div>
                                    ...
                                    <x-heroicon-o-arrow-down class="w-6 h-6 mr-3" wire:loading.remove
                                        wire:target="updatePassword" />
                                </div>
                                <div>
                                    <span wire:loading wire:target="updatePassword">Updating...</span>
                                    <x-heroicon-o-arrow-path class="w-6 h-6 mr-3 animate-spin" wire:loading
                                        wire:target="updatePassword" />
                                </div>
    ...
    <div class="bg-white dark:bg-gray-800 shadow-xl rounded-xl p-6 border border-gray-100 dark:border-gray-700 mt-6">
        <h3 class="text-xl font-semibold text-gray-800 dark:text-white mb-4">My Shifts</h3>

        <div class="space-y-3">
            @if (count($shifts))
                @foreach ($shifts as $shift)
                    <div class="p-3 bg-gray-50 dark:bg-gray-900 rounded-md border flex items-center justify-between">
                        <div>
                            <div class="text-sm font-medium text-gray-800 dark:text-white">
                                {{ $shift['shift_type'] ?? 'Shift' }}
                            </div>
                            <div class="text-xs text-gray-600 dark:text-gray-300">
                                {{ \Illuminate\Support\Carbon::parse($shift['shift_date'])->format('M d, Y') }}
                                —
                                @php
                                    ...
                                    try {
                                        $startFmt = $start
                                            ...
                                            : $start;
                                        $endFmt = $end ? \Illuminate\Support\Carbon::parse($end)->format('H:i') : $end;
                                    } catch (\Throwable $e) {
                                        $startFmt = $start;
                                        $endFmt = $end;
                                    }
                ...
                @endforeach
            @else
                <p class="text-sm text-gray-600 dark:text-gray-300">No shifts scheduled.</p>
            @endif
        </div>
    </div>
</main>
