...
<aside x-cloak
    ...
    <div class="h-16 flex items-center px-4 border-b border-gray-100 dark:border-gray-800 relative flex-shrink-0">
        <a href="{{ route('nurse.dashboard') }}" class="flex items-center gap-3 overflow-hidden whitespace-nowrap">
            ...
            {{-- Label: Visible on Mobile always, toggled on Desktop --}}
            <div class="flex flex-col transition-opacity duration-300"
                 :class="{
                    'opacity-100': sidebarExpanded || mobileOpen,
                    'lg:opacity-0 lg:w-0 lg:hidden': !sidebarExpanded && !mobileOpen
                 }">
        ...
        <button @click="toggleSidebar()"
                class="hidden lg:flex absolute -right-3 top-1/2 -translate-y-1/2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-500 hover:text-rose-600 rounded-full p-1 shadow-md z-50 transition-transform duration-300 focus:outline-none"
            ...
            <li x-data="{ tooltip: false }" @mouseleave="tooltip = false" class="relative">
                <a href="{{ route($item['route']) }}"
                   ...
                   class="flex items-center rounded-lg px-3 py-2.5 transition-all duration-200 group relative
                    ...
                    {{-- Text Label: Always show on Mobile, toggle on Desktop --}}
                    ...
                    <div x-show="!sidebarExpanded && !mobileOpen && tooltip" x-cloak class="absolute left-14 z-50 px-2 py-1 bg-gray-900 text-white text-xs rounded shadow-lg whitespace-nowrap pointer-events-none">{{ $item['label'] }}</div>
                </a>
            </li>
            @endforeach
        </ul>
