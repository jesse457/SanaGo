<aside x-cloak
    ...
    <div class="h-16 flex items-center px-4 border-b border-gray-100 dark:border-gray-800 relative flex-shrink-0">
        <a href="{{ route('lab-technician.dashboard') }}" class="flex items-center gap-3 overflow-hidden whitespace-nowrap">
            <div class="w-10 h-10 rounded-xl bg-gradient-to-tr from-teal-500 to-emerald-600 flex items-center justify-center flex-shrink-0 shadow-lg shadow-teal-500/30 text-white">
                <x-heroicon-m-beaker class="w-6 h-6" />
            </div>
            <div class="flex flex-col transition-opacity duration-300"
                 :class="sidebarExpanded ? 'opacity-100' : 'opacity-0 w-0 hidden'">
                <span class="font-bold text-gray-900 dark:text-white leading-none tracking-tight text-base">Laboratory</span>
                <span class="text-[10px] font-semibold text-gray-500 uppercase tracking-wider mt-0.5">Portal</span>
            </div>
        </a>
        <button @click="toggleSidebar()" class="hidden lg:flex absolute -right-3 top-1/2 -translate-y-1/2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-500 hover:text-teal-600 rounded-full p-1 shadow-md z-50 transition-transform duration-300" :class="{ 'rotate-180': !sidebarExpanded }">
    ...
    <nav class="flex-1 overflow-y-auto overflow-x-hidden py-6 px-3 space-y-1 custom-scrollbar">
        ...
        @php
            $menuItems = [
                ['route' => 'lab-technician.dashboard', 'icon' => 'squares-2x2', 'label' => 'Dashboard'],
                ['route' => 'lab-technician.test-requests', 'icon' => 'clipboard-document-list', 'label' => 'Test Requests'],
                ['route' => 'lab-technician.lab-results', 'icon' => 'beaker', 'label' => 'Lab Results'],
                ['route' => 'lab-technician.manage-lab-definitions', 'icon' => 'clipboard-document-check', 'label' => 'Manage Tests'],
                ['route' => 'lab-technician.feedbacks', 'icon' => 'chat-bubble-left-right', 'label' => 'Feedbacks'],
            ];
        @endphp
        ...
        <ul class="space-y-1">
            ...
            <li x-data="{ tooltip: false }" @mouseleave="tooltip = false" class="relative">
                <a href="{{ route($item['route']) }}" wire:navigate @mouseenter="if(!sidebarExpanded) tooltip = true"
                   class="flex items-center rounded-lg px-3 py-2.5 transition-all duration-200 group relative {{ $isActive ? 'bg-teal-50 dark:bg-teal-900/20 text-teal-700 dark:text-teal-400 font-semibold' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white' }}">
                    <div class="flex-shrink-0 transition-colors {{ $isActive ? 'text-teal-600 dark:text-teal-400' : 'text-gray-400 group-hover:text-gray-600' }}">
    ...
    <div class="p-4 border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900">
        <div class="flex items-center gap-3 transition-all duration-300" :class="sidebarExpanded ? 'justify-start' : 'justify-center'">
            <div class="relative flex-shrink-0">
                ...
                @else
                    <div class="w-9 h-9 rounded-full bg-teal-100 dark:bg-teal-900 text-teal-700 dark:text-teal-300 flex items-center justify-center font-bold text-xs border border-teal-200">{{ substr(auth()->user()->name, 0, 1) }}</div>
                @endif
            </div>
