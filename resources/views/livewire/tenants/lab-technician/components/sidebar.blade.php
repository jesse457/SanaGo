...
<aside x-cloak
    ...
    <div class="flex items-center justify-between p-4 lg:hidden border-b border-gray-100 dark:border-gray-700">
        ...
        <button @click="mobileOpen = false"
            class="p-2 rounded-full text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700">
    ...
    <!-- User Info Section -->
    <div
        class="user-info-section py-6 border-b border-gray-100 dark:border-gray-700 transition-all duration-300 flex flex-col items-center">
        <!-- Avatar -->
        ...
        <div x-show="sidebarExpanded" x-transition:enter="transition ease-out duration-200 delay-100"
    ...
    <nav class="flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar py-4 space-y-1">
        ...
        <ul class="space-y-1 px-3">
            @php
                ...
                $menuItems = [
                    [
                        'route' => 'lab-technician.dashboard',
                        'icon' => 'home',
                        'label' => __('lab-technician.dashboard_title') ?? 'Dashboard',
                    ],
                    [
                        'route' => 'lab-technician.test-requests',
                        'icon' => 'test-requests',
                        'label' => __('lab-technician.test_requests') ?? 'Shifts',
                    ],
                    [
                        'route' => 'lab-technician.manage-lab-definitions',
                        'icon' => 'manage-lab-definitions',
                        'label' => __('lab-technician.manage_lab_definitions') ?? 'Manage Lab Tests',
                    ],
                    [
                        'route' => 'lab-technician.lab-results',
                        'icon' => 'lab-results',
                        'label' => __('lab-technician.lab-results') ?? 'Lab Results',
                    ],
                    [
                        'route' => 'lab-technician.feedbacks',
                        'icon' => 'feedbacks',
                        'label' => __('lab-technician.feedbacks') ?? 'Feedbacks',
                    ],
                ...
                <li x-data="{ tooltip: false }" @mouseleave="tooltip = false">
                    <a href="{{ route($item['route']) }}" wire:navigate
                        ...
                        <div class="flex-shrink-0 transition-colors duration-200">
                            ...
                            @elseif($item['icon'] == 'test-requests')
                                <x-heroicon-o-clipboard-document-list class="w-6 h-6" />
                            @elseif($item['icon'] == 'manage-lab-definitions')
                                <x-heroicon-o-clipboard-document-check class="w-6 h-6" />
                            @elseif($item['icon'] == 'feedbacks')
                                <x-heroicon-o-document-text class="w-6 h-6" />
                            @elseif($item['icon'] == 'lab-results')
                        ...
                        <div x-show="!sidebarExpanded && tooltip" x-transition.opacity.duration.200ms
    ...
    <div class="p-3 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
        <form method="POST" action="{{ route('auth.logout') }}">
            @csrf
            <button type="submit" x-data="{ tooltip: false }" @mouseenter="if(!sidebarExpanded) tooltip = true"
                ...
                <span x-show="sidebarExpanded" class="whitespace-nowrap transition-all duration-300">
