<div class="max-w-3xl mx-auto p-4 sm:p-6">
    <div class="bg-white dark:bg-gray-900 shadow-sm ring-1 ring-gray-200 dark:ring-gray-800 rounded-xl overflow-hidden">
        <div class="px-4 sm:px-6 py-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
            <div>
                <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ __('feedback.title') }}</h2>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('feedback.subtitle') }}</p>
            </div>
            <span
        ...
        @if ($submitted)
            <div
                ...
                <p class="text-sm">{{ __('feedback.success_message') }}</p>
            </div>
        @endif
        ...
        <form wire:submit.prevent="submit" class="px-4 sm:px-6 py-6 space-y-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="col-span-1 sm:col-span-2">
                    <label for="subject" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('feedback.label_subject') }}</label>
                    <input id="subject" type="text" wire:model.defer="subject"
                        placeholder="{{ __('feedback.placeholder_subject') }}"
                ...
                <div>
                    <label for="category" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('feedback.label_category') }}</label>
                    <select id="category" wire:model.defer="category"
                        class="mt-1 block w-full rounded-lg border border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                        <option value="issue">{{ __('feedback.option_issue') }}</option>
                        <option value="suggestion">{{ __('feedback.option_suggestion') }}</option>
                        <option value="general">{{ __('feedback.option_general') }}</option>
                        <option value="other">{{ __('feedback.option_other') }}</option>
                    </select>
                    @error('category')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    <label for="priority" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('feedback.label_priority') }}</label>
                    <div class="mt-1">
                        <div class="grid grid-cols-4 gap-2">
                            @php 
                                $priorities = [
                                    'low' => __('feedback.priority_low'),
                                    'normal' => __('feedback.priority_normal'),
                                    'high' => __('feedback.priority_high'),
                                    'urgent' => __('feedback.priority_urgent')
                                ]; 
                            @endphp
                ...
                <div>
                    <label for="department" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('feedback.label_department') }}</label>
                    <input id="department" type="text" wire:model.defer="department"
                        placeholder="{{ __('feedback.placeholder_department') }}"
                        class="mt-1 block w-full rounded-lg border border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-primary-500 focus:border-primary-500" />
                    @error('department')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div>
                <label for="message" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('feedback.label_message') }}</label>
                <textarea id="message" rows="6" wire:model.defer="message"
                    placeholder="{{ __('feedback.placeholder_message') }}"
                ...
                <div class="mt-2 text-xs text-gray-500 dark:text-gray-400">
                    {{ __('feedback.tip_message') }}
                </div>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('feedback.label_attachments') }}</label>
                <div
                    ...
                    <p>{{ __('feedback.drag_drop') }}</p>
                    <p class="mt-1 text-xs">{{ __('feedback.attachment_note') }}</p>
                </div>
                    ...
                    @foreach ($attachments as $i => $file)
                        <div
                            ...
                            <button type="button" class="text-red-600 hover:text-red-700"
                                wire:click="$set('attachments', array_values(collect($attachments)->except($i)->toArray()))">{{ __('feedback.btn_remove') }}</button>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="flex items-center justify-between">
                ...
                <div class="flex  gap-3">
                    <button type="button"
                        ...
                        wire:click="$refresh">{{ __('feedback.btn_start_over') }}</button>
                    <button type="submit"
