...
<header x-data="{
...
}" @scroll.window="handleScroll()" @keydown.escape.window="mobileMenuOpen = false"
    ...
    <nav class="max-w-7xl mx-auto px-6 h-[72px] flex justify-between items-center">
        ...
        @php
            ...
            // Styles
            $baseLink =
                'text-sm font-medium transition-all duration-200 relative py-1 hover:text-blue-600 dark:hover:text-blue-400';
            $activeLink =
                "text-blue-600 dark:text-blue-400 after:content-[''] after:absolute after:bottom-[-2px] after:left-0 after:w-full after:h-0.5 after:bg-blue-600 after:rounded-full";
        ...
        <div class="hidden lg:flex items-center gap-4">
            ...
            <a href="{{ route('book-demo') }}"
                class="group relative inline-flex h-10 overflow-hidden rounded-full p-[1px] focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 focus:ring-offset-slate-50">
                <span
                    class="absolute inset-[-1000%] animate-[spin_2s_linear_infinite] bg-[conic-gradient(from_90deg_at_50%_50%,#E2CBFF_0%,#393BB2_50%,#E2CBFF_100%)]"></span>
                <span
                    class="inline-flex h-full w-full cursor-pointer items-center justify-center rounded-full bg-white dark:bg-slate-950 px-5 py-1 text-sm font-bold text-slate-900 dark:text-white backdrop-blur-3xl transition-colors group-hover:bg-slate-50 dark:group-hover:bg-slate-900">
                    {{ __('navbar.book_now') }}
                </span>
            </a>
        </div>
        ...
        <div class="lg:hidden flex items-center gap-4">
            <button @click="mobileMenuOpen = !mobileMenuOpen"
                class="text-slate-600 dark:text-slate-300 p-2 rounded-md hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
    ...
    <div x-show="mobileMenuOpen" x-transition:enter="transition ease-out duration-200"
        ...
        <div class="px-6 py-6 space-y-2 max-h-[calc(100vh-80px)] overflow-y-auto">
            ...
            @foreach ($navLinks as $link)
                <a wire:navigate href="{{ route($link['route']) }}" @click="mobileMenuOpen = false"
            ...
            <div class="pt-6 mt-4 border-t border-slate-200 dark:border-slate-800 flex flex-col gap-4">
                <!-- Mobile Dark Mode -->
                <button @click="toggleDarkMode()"
                    class="w-full flex items-center justify-between px-4 py-3 rounded-xl bg-slate-100 dark:bg-slate-800/50 text-slate-900 dark:text-white font-medium hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors">
                ...
                <!-- Mobile CTA -->
                <a href="{{ route('book-demo') }}"
                    class="w-full py-3 rounded-xl bg-blue-600 text-white font-bold text-center hover:bg-blue-700 transition-colors shadow-lg shadow-blue-600/20">
                    {{ __('navbar.book_now') }}
                </a>
            </div>
        </div>
